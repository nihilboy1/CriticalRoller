...
const ResultCard = React.forwardRef<HTMLDivElement, ResultCardProps>(
  (props, ref) => {
    const { isRolling, mode, cardCount } = props;

    const renderContent = () => {
      if (isRolling) {
        return <RollingState mode={mode} cardCount={cardCount} />;
      }
    ...
    const borderClass =
      isRolling || props.card ? activeBorderClass : "border-[#2A2A2A]";
